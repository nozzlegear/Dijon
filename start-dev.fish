...
if test (sudo docker ps -a -f "name=$containerName" -q)
    echo "Starting sql database container..."
    sudo docker start "$containerName"; or exit 1
else
    echo "Sql database container does not exist, creating it..."
    echo "Using sql password $sqlPassword"
    sudo docker run -d -it --name "$containerName" -e "SA_PASSWORD=$sqlPassword" -e "ACCEPT_EULA=Y" -p "$containerPort:1433" -v "$PWD/volumes/mssql:/var/opt/mssql" mcr.microsoft.com/mssql/server:2017-latest-ubuntu; or exit 1
end 
